...
============================================================================== */
    ...
    $.fn.dropdown = function () {
        ...
        function dropdown(element) {
            this.dd = element;
            this.placeholder = this.dd.find('.btn-dropdown--text');

            this.opts = this.dd.find('a');
            this.val = '';
            this.index = -1;
            if ($self.hasClass('select')) {
                this.select = this.dd.find('input');
            }
            this.ddList = this.dd.find('.btn-dropdown--dropdown');
            var ddListHeight = this.ddList.outerHeight();
            if (ddListHeight > 350) {
                this.ddList.addClass('long-dropdown');
            }
            this.initEvents();
        }
        ...
        dropdown.prototype = {
            initEvents: function () {
                ...
                obj.opts.on('click', function (e) {
                    e.preventDefault();
                    obj.onSelectOption($(this));
                });

                // list item
                var dropdownArray = [];
                this.opts.each(function () {
                    var thisValue = $(this).html().toLowerCase();
                    dropdownArray.push(thisValue);
                });
                var keyboardVal = null;
                $(document).keydown(function (e) {
                    if (obj.dd.hasClass('active')) {
                        var ddaLength = dropdownArray.length;
                        var newKeyboardVal = keyboardArray[e.which];
                        if (keyboardVal == newKeyboardVal) {
                            for (var i = 0; i < ddaLength; i++) {
                                if (dropdownArray[i].indexOf(keyboardVal) == 0) {
                                    if (!obj.ddList.find('li:eq(' + i + ') a').hasClass('focused')) {
                                        obj.ddList.find('li:eq(' + i + ') a').focus().addClass('focused');
                                        i = ddaLength;
                                    }
                                }
                            }
                        } else {
                            keyboardVal = keyboardArray[e.which];
                            for (var i = 0; i < ddaLength; i++) {
                                if (dropdownArray[i].indexOf(keyboardVal) == 0) {
                                    obj.opts.removeClass('focused');
                                    obj.ddList.find('li:eq(' + i + ') a').focus().addClass('focused');
                                    i = ddaLength;
                                }
                            }
                ...
                if (selectValue) {
                    var selectedElement = this.dd.find('a[value=' + selectValue + ']');
                    if (selectedElement.length) this.onSelectOption(selectedElement);
                }
            },
            onSelectOption: function (option) {
                ...
                if (this.dd.hasClass('select')) {
                    this.valSelect = option.attr('value');
                    this.select.val(this.valSelect);
                }
            },
...
var canvasPrehome = function () {
    ...
    var launch = function launch() {
        ...
        var opts = {
            particleColors: ["#ffd94a", "#fff"],
            particleAmount: 4,
            particleAmountVariant: 8,
            defaultSpeed: 0.05,
            variantSpeed: 0.1,
            defaultRadius: 2,
            variantRadius: 16
        ...
        Particle = function Particle(xPos, yPos) {
            ...
            this.color = opts.particleColors[~~(Math.random()*opts.particleColors.length)];
        ...
        function setup() {
            particles = [];
            resizeReset();
            for (var i = 0; i < (opts.particleAmount + Math.random() * opts.particleAmountVariant); i++) {
        ...
        var canvasBody = document.getElementById("prehome--dots"),
...
var site = (function() {
    ...
    var init = function() {
        ...
        if (tools.getInternetExplorerVersion() == 10) {
            $('html').addClass('ie10');
        }
     ...
     * Resize handler
     */
    var bearstrokes = function(){
        // position strokes
        var $bearstrokes = $('.bearstrokes');
        var windowWidth = $(window).width();
        var strokesWidth = $bearstrokes.outerWidth();
        var calc = (windowWidth - strokesWidth)/2;
        $bearstrokes.css('left', calc);
        $('.bearnav--bg').css('left', calc);

        setTimeout(bearstrokes, 1000);
    }
    ...
    var menuBurger = function() {
        ...
        var $burger = $('.bearheader--burger');
        var $bearnav = $('.bearnav');
        var $links = $bearnav.find('a');

        var canChange = true;
        var deployed = false;

        // add content="text" to each link
        $links.each(function(){
            var $self = $(this);
            $self.attr('content', $self.html());
        });
        ...
        // on click close
        $bearnav.find('.bearnav--close').on('click', menuHide);

        // show
        function menuShow() {
            deployed = true;
            $('html').addClass('menu--deployed menu--active');
            ...
            setTimeout(function(){
                $('html').addClass('menu--deployed__end');
                canChange = true;
            }, 700);
        }
        ...
        function menuHide() {
            deployed = false;
            $('html').removeClass('menu--deployed menu--deployed__end');
            setTimeout(function(){
        ...
        // hide menu
        $('.bearnav--nav a').on('click', function(){
            setTimeout(function(){
                canChange = true;
                menuHide();
            }, 250);
    ...
    var themeChanger = function() {
        ...
        $('body').on('click', '.bearheader--theme', changeTheme);

        var updateTheme = function(thisTheme) {
            $('html').removeClass('theme--polar theme--black');
            $('html').addClass('theme--' + thisTheme);

            var dateNow = new Date().getTime();

            if (tools.isStorageSupported() && !tools.simplifiedVersion()) {
                localStorage.setItem('bear-theme', thisTheme);
                localStorage.setItem('bear-theme-date', dateNow);
            }

            theme = thisTheme;

            if (!tools.simplifiedVersion() || tools.getInternetExplorerVersion() != 10) {
                homeVideo();
            }
        ...
        // localstorage
        var dateNow = new Date().getTime();
        if (tools.isStorageSupported()) {
            var userThemeDate = parseInt(localStorage.getItem('bear-theme-date'));
            var userTheme = localStorage.getItem('bear-theme');
        } else {
            var userThemeDate = parseInt(dateNow+1);
            var userTheme = 'polar';
        }
        ...
        // if userTheme exist
        // AND
        // if old visit is older than x time
        if (userTheme !== null && dateNow < userThemeDate + time) {
            updateTheme(userTheme);
            $('.prehome').remove();
            $('.prehome--bg').remove();
        } else {
            if (tools.simplifiedVersion()) {// mobile
                changeTheme();
                $('.prehome').remove();
                $('.prehome--bg').remove();
            } else {// desktop
                $('html').addClass('prehome-active');
                $('.bearstrokes').appendTo($('.global-container'));

                canvasPrehome.launch();
                ...
                tl.staggerFromTo(
                    $('.prehome--chosen > div > svg, .prehome--chosen > div > span'),
                    0.8,
                    {
                        y: 10,
                        alpha:0,
        ...
        $('body').on('click', '.prehome--polar, .prehome--black', function(){
            var $self = $(this);
            theme = $self.attr('theme');
            updateTheme(theme);
            ...
            tl.call(function(){
                $('html').removeClass('prehome-active');
                $('.prehome').remove();
                $('.prehome--bg').remove();
                ...
                // bearstrokes
                $('.bearstrokes').prependTo($('.scroll-container'));
                var tl = new TimelineLite();
                tl.pause();
                tl.fromTo(
                    $('.bearstrokes'),
    ...
    var smoothScroll = function() {
        ...
        if (Math.round(currentY) != Math.round(destY)) {
            var newY = Math.round(currentY+((destY-currentY)*speed));
            container.css({transform: 'translate3d(0, -'+newY+'px, 0)'});
            beardots.css({transform: 'translate3d(0, -'+newY+'px, 0)'});
        }
        */
    ...
    var smoothScroll = function() {
        ...
        $(window).on('scroll', _throttle(function(e){
            scrollTop = $(window).scrollTop();
            //TweenLite.to($('.creanobackground'), .75, { y:(-scrollTop/5), ease:Power3.easeOut, force3D : true });
            TweenLite.to(container, .75, { y:-scrollTop, ease:Power3.easeOut, force3D : true });
            TweenLite.to(beardots, .75, { y:-scrollTop, ease:Power3.easeOut, force3D : true });
        }, 20));
    }
    */
    ...
    var pageInit = function() {
        ...
        if (tools.simplifiedVersion() || tools.getInternetExplorerVersion() == 10) {
            $('.home--bg video, .team--item-img video').remove();
        } else {
            // home video
            homeVideo();
        }
        ...
        // talents show more/less
        talentsMoreLess();


        if (pageSlug === 'Contact' && oldPage !== null) {
            // location.reload(true);
        }

        //if (location.pathname.indexOf('/contact') || location.pathname.indexOf('/nous-contacter'));
        /*
        ...
        */
        ...
        $.get("js/scripts.js", function(data) {
            //$('.wpcf7').wpcf7InitForm();
    ...
    var talentsMoreLess = function() {
        var $module = $('.talents');
        $module.each(function(){
            var $self = $(this);
            var $showmore = $self.find('.showmore');
            var isActive = false;
            $showmore.on('click', function(){
                $showmore.toggleClass('active');
                isActive = !isActive;

                $self.find('.talents--item').addClass('transi');
                setTimeout(function(){
                    $self.find('.talents--item').removeClass('transi');
                }, 500);

                if (isActive) {
                    $self.find('.talents--item').removeClass('inactive');
                } else {
                    $self.find('.talents--list').each(function(){
                        $(this).find('.talents--item').each(function(index){
                            if (index >= 4) {
     ...
     * gridifyTalents
     */
    var gridifyTalents = function() {

        var $cont = $('.talents-container');
        if ($cont.length) {
            $cont.each(function(){

                var $self = $(this);
                $self.removeClass('talents-container__reassort');
                var $items = $self.find('.talents--item');
                var itemsLength = $items.length;

                if (itemsLength>1) {

                    var contWidth = $self.outerWidth();
                    var itemWidth = $items.eq(0).outerWidth(true);

                    var row = Math.floor(contWidth / itemWidth);

                    var items_per_col = new Array();
                    var min_items_per_col = Math.floor($items.length / row);
                    var difference = $items.length - (min_items_per_col * row);
                    ...
                    for (var i = 0; i < row; i++) {
                        $self.append($('<div/>').addClass('talents--list'));
                    }
                    $items.each(function(index){
                        var $item = $(this);
                        $item.appendTo($self.find('.talents--list').eq(index%items_per_col.length));

                        $item.removeClass('talents--item__c0');
                        $item.removeClass('talents--item__c1');
                        $item.removeClass('talents--item__c2');
                        $item.addClass('talents--item__c' + index % 3);
                    });

                    $self.addClass('talents-container__reassort');
                    $self.find('.talents--list:nocontent').remove();
                }
            });
        }
    }
    ...
    var indexProjects = function() {
        var $cont = $('.projects-container');
        if ($cont.length) {
            $cont.each(function(){
                var $items = $(this).find('.projects--item');
                $items.each(function(index) {
    ...
    var gridifyProjects = function() {
        ...
        if ($cont.length) {
            ...
            $cont.each(function(){
                ...
                var $self = $(this);
                $self.removeClass('projects-container__reassort');
                var $items = $self.find('.projects--item');
                var itemsLength = $items.length;
                ...
                if (itemsLength>1) {
                    ...
                    for (var i = 0; i < row; i++) {
                        $self.append($('<div/>').addClass('projects--list'));
                    }
                    ...
                    $items.each(function(index){
                        var $item = $(this);
                        $item.appendTo($self.find('.projects--list').eq(index%items_per_col.length));

                        $item.removeClass('projects--item__c0');
                        $item.removeClass('projects--item__c1');
                        $item.removeClass('projects--item__c2');
                        $item.addClass('projects--item__c' + index % 3);

                        //$item.find('.projects--item-excerpt').html(index);//debug
                    });

                    $self.addClass('projects-container__reassort');
                    $self.find('.projects--list:nocontent').remove();
                }
            });
        }
    }
     ...
     * gridifyAwards
     */
    var gridifyAwards = function() {

        var $cont = $('.awards-container');
        if ($cont.length) {

            $cont.each(function(){
                var $self = $(this);
                $self.removeClass('awards-container__reassort');
                var $items = $self.find('.awards--item');
                var itemsLength = $items.length;
                ...
                if (itemsLength>1) {
                    ...
                    for (var i = 0; i < row; i++) {
                        $self.append($('<div/>').addClass('awards--list'));
                    }
                    $items.each(function(index){
                        var $item = $(this);
                        $item.appendTo($self.find('.awards--list').eq(index%items_per_col.length));

                        $item.removeClass('awards--item__c0');
                        $item.removeClass('awards--item__c1');
                        $item.removeClass('awards--item__c2');
                        $item.addClass('awards--item__c' + index % 3);
                    });

                    $self.addClass('awards-container__reassort');
                    $self.find('.awards--list:nocontent').remove();
                }
            });
        }
    }
     ...
     * gridifyTeam
     */
    var gridifyTeam = function() {

        var $cont = $('.team-container');
        if ($cont.length) {


            $cont.each(function(){
                var $self = $(this);
                $self.removeClass('team-container__reassort');
                var $items = $self.find('.team--item');
                var itemsLength = $items.length;
                ...
                if (itemsLength>1) {
                    ...
                    $items.each(function(index){
                        ...
                        var $imgPolar      = $self.find('.team--item-polar');
                        var $imgBlack      = $self.find('.team--item-black');

                        if (theme === 'polar') {
                            $imgPolar.attr('src', $imgPolar.attr('data-src'));
                        } else if (theme === 'black') {
                            $imgBlack.attr('src', $imgBlack.attr('data-src'));
                        }

                        $self.addClass('team--item__c' + index % 3);
                        /*
                        var itemHeight = $self.find('.team--item-img').outerHeight();
                        var itemWidth = $self.find('.team--item-img').outerWidth();

                        var $imgs = $self.find('img');
                        $imgs.each(function(){
                            var $img = $(this);
                            var state = $img.outerHeight() <= itemHeight &&
                            console.log(state);
                            $img.outerWidth() >= itemWidth;

                            $img[0].classList[state ? 'add' : 'remove']('team--item-img__max');
                        });
                        */
                    });

                    $self.addClass('team-container__reassort');
                    $self.find('.team--list:nocontent').remove();
                }
            });
        }
    }
     ...
     * teamItem
     */
    var teamItemEnter = function(e){
        if (!tools.simplifiedVersion()) {
            var $el = $(this);
            var $vid = $el.find('.team--item-'+theme+'-hover');
            if ($vid.length && $vid[0].paused && tools.getInternetExplorerVersion() != 10) {
                $vid[0].currentTime = 0;
                $vid[0].play();
            }
        }
    }
    var teamItemLeave = function(e){
        if (!tools.simplifiedVersion()) {
            var $el = $(this);
            var $vid = $el.find('.team--item-'+theme+'-hover');
            if ($vid.length && tools.getInternetExplorerVersion() != 10) {
                $vid[0].pause();
            }
        }
    }
    ...
    var postSs = function() {
        ...
        if ($ss.length) {
            ...
            $ss.each(function(){
                ...
                var $self   = $(this);
                var $slider = $self.find('.post-ss--ss');
                var $dots   = $self.find('.post-ss--dots li');
                var $slides = $self.find('.post-ss--ss li');
                ...
                var setHeight = function(){
                    ...
                    $img.each(function(){
                        ...
                        if (selfHeight > contHeight) {
                            contHeight = selfHeight;
                        }
                    });
                ...
                $dots.on('click', function(){
                    ...
                    if (!$self.hasClass('current') && canChangeSlide) {
                ...
                // update slides
                var updateSlide = function(id) {
                    var $old = $slides.filter('.current');
                    var $new = $slides.eq(id);
     ...
     * teamover
     */
    var teamover = function() {

        var $teamover = $('.teamover');
        $teamover.appendTo('body');

        var teamOverActive = false;
        var indexCurrent = 0;
        var direction = 'right';
        var canChangeMember = true;
        var $items = $teamover.find('.teamover--item');


        // change slide
        var updateSlide = function(indic){
            var $old = $teamover.find('.teamover--item__current');
            hideItem($old);

            if (indic != 'none') {
                var $current = $items.eq(indexCurrent);
                showItem($current);
            }
        };
        ...
        var hideItem = function($el) {
            $el.removeClass('teamover--item__active');
            $el.removeClass('teamover--item__current');
            ...
            $el.removeClass('teamover--item__current');
            $el.addClass('teamover--item__active');
            ...
            tl.staggerFromTo(
                $el.find('.teamover--item-thumb, .teamover--item-desc'),
                .4,
                {
                    y: 0,
                    alpha:1,
            ...
            tl.staggerFromTo(
                $el.find('.teamover--item-title, .teamover--item-markup > *'),
                .4,
                {
                    y: 0,
                    alpha:1,
        ...
        var showItem = function($el) {
            ...
            $el.addClass('teamover--item__current');
            $el.addClass('teamover--item__active');
            ...
            // set image size
            if (tools.isSmartphone() && window.matchMedia("(min-width: 661px)").matches) {
                var $imgc = $el.find('.teamover--item-thumb');
                var $desc = $el.find('.teamover--item-desc');
                var descHeight = $desc.outerHeight();
                var imgcWidth = $imgc.outerWidth();
                var $img = $imgc.find('img');
                var state = $img.outerHeight() <= descHeight && $img.outerWidth() >= imgcWidth-1;
                $img[0].classList[state ? 'add' : 'remove']('teamover--img__max');
                $imgc.css('height', descHeight);
            }
            ...
            /*
            tl.fromTo(
                $el.find('.teamover--item-thumb img'),
                10,
                {
                    scale: 1,
                }, {
        ...
        // update index
        var updateId = function(dirId) {
            if (canChangeMember) {
                canChangeMember = false;
                var num = dirId || 0;

                indexCurrent = indexCurrent + num;

                if (indexCurrent < 0) {
                    indexCurrent = $items.length - 1;
                } else if (indexCurrent >= $items.length) {
                    indexCurrent = 0;
                }
        ...
        // show teamover
        var showTeamover = function(){
            if (tools.isSmartphone()) {
                scrollOnTeamover = $(window).scrollTop();
                $('html, body').scrollTop(0);
            }

            teamOverActive = true;
            $('html').addClass('teamover--deployed teamover--active');
            ...
            tl.call(function(){
                $('html').addClass('teamover--deployed__end');
            });
        ...
        // hide teamover
        var hideTeamover = function(){
            teamOverActive = false;
            $('html').removeClass('teamover--deployed__end');
            updateSlide('none');
            ...
            tl.call(function(){
                $('html').removeClass('teamover--deployed teamover--active');
                if (tools.isSmartphone()) {
                    $('html, body').scrollTop(scrollOnTeamover);
                    initScrollAnimations();
                }
            });
        ...
        // controls
        $('body').off('click', '.team--item');
        $('body').on('click', '.team--item', function(){
            var $self = $(this);
            var member = $self.attr('team-member');
            indexCurrent = $teamover.find('[team-member="'+member+'"]').index();
            showTeamover();
        });

        $('.teamover--nav__prev').on('click', function(){
            direction = 'left';
            updateId(-1);
        });
        $('.teamover--nav__next').on('click', function(){
            direction = 'right';
            updateId(+1);
        });
        $('.teamover--close, .teamover--overlay').on('click', function(){
            hideTeamover();
        });
        ...
        $(window).on('keydown', function(e) {
            if (teamOverActive) {
                if (e.keyCode == 37) {// left
                    $teamover.find('.teamover--nav__prev').trigger('click');
                }
                if (e.keyCode == 39) {// right
                    $teamover.find('.teamover--nav__next').trigger('click');
                }
                if (e.keyCode == 27) {// escape
                    $teamover.find('.teamover--close').trigger('click');
                }
            }
        });
    }
    ...
    var contactCtrl = function(){
        $('.superform .btnca').addClass('btnca__active btnca__end')
    };
    ...
    var choiceHide = function(){
        ...
        tl.call(function(){
            $('.choices').addClass('choices__inactive');
            $('.superform').eq(index).addClass('superform__active');
        });
    ...
    var choiceShow = function(){
        ...
        tl.call(function(){
            $('.superform').removeClass('superform__active');
            $('.choices').removeClass('choices__inactive');
        });
     ...
     * contact dropdown
     */
    var contactDropdownLink = function() {
        var val = $(this).attr('value');
        $('.superform').removeClass('superform__active');
        $('.superform').eq(val).addClass('superform__active');
    };
    var contactDropdown = function() {
        ...
        if (window.matchMedia("(max-width: 800px)").matches) {
            $('.btn-dropdown--dropdown').find('a').eq(0).trigger('click');
        }
    };
    ...
    var sended = function(){
        ...
        tl.call(function(){
            $('.superform').removeClass('superform__active');
            $('.bearform-valid').addClass('is-active');
        });
        ...
        smoothScrollTo(0);
        setTimeout(function(){
            $('.bearheader--logo a').trigger('click');
        }, 2000);
    ...
    var generateDots = function() {
        ...
        var docHeight = $('.scroll-container').outerHeight();

        var randDots = Math.round((Math.random()*(maxDots-minDots) + minDots) * (docHeight / winHeight));

        var $beardots = $('.beardots');
        $beardots.html('');

        for (var i = randDots - 1; i >= 0; i--) {
            var $dot = $('<div/>');
            $dot.addClass('beardots--dot scroll-parallax');

            var randAmp = Math.random()*.75 + 0.25;
            $dot.attr('data-level', randAmp);

            // pos
            var x = Math.round(Math.random()*docWidth);
            var y = Math.round(Math.random()*docHeight);
            $dot.css('left', x);
            $dot.css('top', y);

            // color
            var color = Math.round(Math.random()*1);
            $dot.addClass('beardots--dot__c' + color);

            // size
            if (i % 12 == 0) {
                var size = 100;
            } else {
                var size = Math.round(Math.random()*(maxSize-minSize)) + minSize;
            }
            ...
            // inject
            $dot.appendTo($beardots);
        }


        // super big
        $('.beardots--dot').eq(20).css('width', '380px');
        $('.beardots--dot').eq(20).css('height', '380px');
        if (Math.floor(Math.random() * 2) == 0) {
            $('.beardots--dot').eq(20).css('left', - (Math.round(Math.random()*(310-220)) + 220) );
        } else {
            $('.beardots--dot').eq(20).css('left', docWidth - (Math.round(Math.random()*(160-90)) + 90));
        }


        //console.log($('.beardots--dot').length + ' dots');
     ...
     */
    var addClassSlug = function() {
        // add html page class
        $.each(slugs, function(){
            $('html').removeClass('Page'+this);
        });
        $('html').addClass('Page'+pageSlug);
    }
     ...
     */
    var bindEvents = function() {
        $('body').on('click', '.bearheader--langs li:not(.current-lang) a, .prehome--langs li:not(.current-lang) a', function(){
            pageLangSwitch = true;
        });
        ...
        // choice contact form
        $('body').on('click', '.choice', choiceHide);
        $('body').on('click', '.superform .btnca', choiceShow);
        $('body').on('click', '.btn-dropdown--dropdown a', contactDropdownLink);

        // team item
        $('body').on('mouseenter', '.team--item', teamItemEnter);
        $('body').on('mouseleave', '.team--item', teamItemLeave);

        // bearpager
        $('body').on('mouseenter mouseleave mousemove', '[bearpager]', bearpager);
        $('body').on('click', '[bearpager]', bearpagerClick);

        $('body').on('click', '.homenav--soundwave', function(){
            var $audio = $('#sound--bearideas')[0];
            $audio.currentTime = 0;
            $audio.play();
        });
    }
     ...
     * bearpager
     */
    var timeoutBearpager;
    var bearpagerClick = function(e) {
        var dir = $(this).attr('bearpager');
        var $target = pageSlug === 'Post'?$('.page-container:first .post-header--nav__'+dir+' a').eq(0):$('.page-container:first .footcontrol--cta__'+dir);
        if ($target.length) {
            $target.trigger('click');
            $('.bearpager-indicator').removeClass('active');
        }
    }
    var bearpager = function(e) {

        if (tools.simplifiedVersion() && window.matchMedia("(max-width: 1400px)").matches) {
            return;
        }

        var $self = $(this);
        var $indic = $('.bearpager-indicator');

        clearTimeout(timeoutBearpager);

        var dir = $self.attr('bearpager');

        var mouseX = e.pageX;
        var mouseY = e.pageY;

        // if post or rest of the site
        if (pageSlug === 'Post') {
            var $target = $('.page-container:first .post-header--nav__'+dir+' a').eq(0);
            $self.attr('cursor', 'pointer');
            $indic.find('span').html($('.page-container:first .indicator-project__'+dir).html());
        } else {
            var $target = $('.page-container:first .footcontrol--cta__'+dir);
            if ($target.length) {
                $self.attr('cursor', 'pointer');
            } else {
                $self.attr('cursor', '');
            ...
            $indic.find('span').html($target.find('span').html());
        }
        ...
        var calc = dir==='right'?mouseX-$indic.outerWidth():mouseX;
        if (e.type === 'mouseenter') {
            $indic.addClass('active');
            TweenMax.set($indic, {x:calc, y:mouseY});
        } else if (e.type === 'mouseleave') {
            $indic.removeClass('active');
            timeoutBearpager = setTimeout(function(){
                $indic.attr('dir', '');
            }, 250);
        } else {// mousemove
        }
        ...
        // set pos
        TweenMax.to($indic, 1, {x:calc, y:mouseY, ease:Expo.easeOut});
    }
    ...
    var linkHandler = function(e) {
        ...
        */
        ...
        if (url.indexOf(thishost) === 0) {
            ...
            if (window.location.href.trimSlash().trimHttp() === url) {
                return;
            }
    ...
    var pageOutro = function() {
        ...
        var el = $('.page-container').first().add('.beardots');
    ...
    var pageCreate = function(data) {
        oldPage = $('.page-container:first').attr('data-page').toCamel();

        // Convert data to HTML
        var html = $('<div/>').html(data);

        // remove teamover before insert
        removeTeamover();
        ...
        if (typeof(ga) !== 'undefined') {
            ga('set', { page: window.location.pathname, title: html.find('title').html() });
            ga('send', 'pageview');
        }

        // switch content
        //$('.wp-foot').replaceWith(html.find('.wp-foot'));
        //$('.bearheader').replaceWith(html.find('.bearheader'));
        //$('.bearnav').replaceWith(html.find('.bearnav'));
        $('.bearheader--langs').replaceWith(html.find('.bearheader--langs'));
        //$('.teamover--content').remove();



        // Get page controller
        pageSlug = newContainer.attr('data-page').toCamel();
    ...
    var showLoader = function() {
        ...
        tl.staggerFromTo(
            loader.find('.page-loader--loader div'),
            1,
            {
                y: 1000,
            }, {
    ...
    var pageIntro = function() {
        ...
        var el = newContainer.add('.beardots');
    ...
    var resizeContainer = function() {
        ...
        //console.log('set body height', containerHeight);
    ...
    var initScrollAnimations = function() {
        ...
        $('.page-container:first .scroll-reveal').each(function() {
            ...
            scene.on('start', function(e) {
                if (e.scrollDirection != 'REVERSE' && !$('html').hasClass('prehome-active')) {
    ...
    var scrollRevealHandler = function() {
        ...
        if (element.is('.superheader')) {
            ...
            tl.to(
                element.find('.superheader--title > div'),
                0.4,
                {
                    y: '0%',
                    ease: Power3.easeOut
            ...
            tl.to(
                element.find('.superheader--title > span'),
                0.8,
                {
                    y: '0',
                    alpha:1,
            ...
            tl.to(
                element.find('.superheader--subtitle > div'),
                0.4,
                {
                    y: '0%',
                    ease: Power3.easeOut
            ...
            tl.to(
                element.find('.superheader--subtitle > span'),
                0.8,
                {
                    y: '0',
                    alpha:1,
        ...
        } else if (element.is('.supertext-a')) {

            element.addClass('is-revealed');

            // dashed circle
            var randX = Math.round(Math.random() * 200 - 100);
            var randY = Math.round((Math.random() * 200 - 100)/1.5);
            element.find('.supertext-a--icon').css('transform', 'translate('+randX+'%, '+randY+'%)');

            // text
            var splitText = new SplitText(
                element.find('.supertext-a--title span, .supertext-a--subtitle span'),
                {
                    type: 'lines,words',
                    linesClass: 'splitter-line',
                    wordsClass: 'splitter-word'
                }
            );
            var lines = element.find('.splitter-line');
            //var words = element.find('.splitter-word');
        ...
        } else if (element.is('.supertext-b')) {
            ...
            var randX = Math.round(Math.random() * 200 - 100);
            var randY = Math.round((Math.random() * 200 - 100)/1.5);
            element.find('.supertext-b--icon').css('transform', 'translate('+randX+'%, '+randY+'%)');

            // text
            var splitText = new SplitText(
                element.find('.supertext-b--text > div > span'),
                {
                    type: 'lines,words',
                    linesClass: 'splitter-line',
                    wordsClass: 'splitter-word'
        ...
        } else if (element.is('.basic-headline')) {

            element.addClass('is-revealed');

            // text
            var splitText = new SplitText(
                element.find('.basic-headline--text > div > span'),
                {
                    type: 'lines,words',
                    linesClass: 'splitter-line',
                    wordsClass: 'splitter-word'
        ...
        } else if (element.is('.talents-container')) {

            element.addClass('is-revealed');

            var $items = element.find('.talents--item');
            $items.sort(function(a, b) {
                var contentA = $(a).offset().top;
                var contentB = $(b).offset().top;
            ...
            $items.each(function(index){
                ...
                setTimeout(function(){
                    ...
                    // show overlay
                    tl.fromTo(
                        $self.find('.talents--item-overlay'),
                        .5,
                        {
                            y: '101%',
                        }, {
                    ...
                    tl.staggerFromTo(
                        $self.find('.talents--item-inner > div'),
                        0.8,
                        {
                            y: 20,
                            alpha:0,
        ...
        } else if (element.is('.awards-container')) {

            element.addClass('is-revealed');

            var $items = element.find('.awards--item');
            $items.sort(function(a, b) {
                var contentA = $(a).offset().top;
                var contentB = $(b).offset().top;
                return (contentA < contentB) ? -1 : (contentA > contentB) ? 1 : 0;
        ...
        } else if (element.is('.references-container')) {

            element.addClass('is-revealed');

            var $items = element.find('.references--item');
            $items.sort(function(a, b) {
                var contentA = $(a).offset().top;
                var contentB = $(b).offset().top;
                return (contentA < contentB) ? -1 : (contentA > contentB) ? 1 : 0;
            ...
            $items.each(function(index){
                ...
                setTimeout(function(){
                    ...
                    // show overlay
                    tl.fromTo(
                        $self.find('.references--item-overlay'),
                        .5,
                        {
                            y: '101%',
                        }, {
                    ...
                    tl.staggerFromTo(
                        $self.find('.references--item-inner img'),
                        0.8,
                        {
                            y: 20,
                            alpha:0,
        ...
        } else if (element.is('.skills-container')) {


            // text
            var splitText = new SplitText(
                element.find('.skills--desc span'),
                {
            ...
            element.find('.splitter-line').css('opacity', '0');
            element.addClass('is-revealed');


            $('.skills--list').each(function(index){
                var element = $(this);
                ...
                setTimeout(function(){
                    ...
                    tl.to(
                        element.find('.skills--title-overlay'),
                        0.4,
                        {
                            y: '0%',
                            ease: Power3.easeOut
                    ...
                    tl.to(
                        element.find('.skills--title-text'),
                        0.8,
                        {
                            y: '0',
                            alpha:1,
                    ...
                    tl.to(
                        element.find('.skills--title-stroke'),
                        2,
                        {
                            y: '0',
                            alpha:1,
        ...
        } else if (element.is('.territory-container')) {

            element.addClass('is-revealed');

            var pt = new path(element.find('.svg-illubear--path path'));
            pt.init();
            ...
            tl.call(function() {
                setTimeout(pt.draw, 1000);
            });

            tl.staggerFromTo(
                element.find('.svg-illubear--circle path'),
                0.8,
                {
                    scale:0,
                    alpha:0,
            ...
            tl.staggerFromTo(
                element.find('.svg-illubear--forme'),
                2,
                {
                    alpha: 0
                }, {
        ...
        } else if (element.is('.team-container')) {

            element.addClass('is-revealed');

            var $items = element.find('.team--item');
            ...
            $items.each(function(index){
                ...
                setTimeout(function(){
                    ...
                    tl.fromTo(
                        element.find('.team--item-overlay'),
                        0.4,
                        {
                            x: '-101%',
                            alpha: 1,
                    ...
                    tl.call(function() {
                        element.find('.team--item-img').css('opacity', 1);
                    });
        ...
        } else if (element.is('.projects-container')) {

            element.addClass('is-revealed');

            var $items = element.find('.projects--item');
            $items.sort(function(a, b) {
                var contentA = $(a).offset().top;
                var contentB = $(b).offset().top;
                return (contentA < contentB) ? -1 : (contentA > contentB) ? 1 : 0;
            ...
            $items.each(function(index){
                ...
                setTimeout(function(){
                    ...
                    // show overlay
                    tl.fromTo(
                        $self.find('.projects--item-overlay'),
                        .5,
                        {
                            x: '-101%',
                        }, {
        ...
        } else if (element.is('.post-header')) {
            ...
            tl.to(
                element.find('.post-header--title > div'),
                0.4,
                {
                    y: '0%',
                    ease: Power3.easeOut
            ...
            tl.to(
                element.find('.post-header--title > span'),
                0.8,
                {
                    y: '0',
                    alpha:1,
            ...
            tl.to(
                element.find('.post-header--subtitle > div'),
                0.4,
                {
                    y: '0%',
                    ease: Power3.easeOut
            ...
            tl.to(
                element.find('.post-header--subtitle > span'),
                0.8,
                {
                    y: '0',
                    alpha:1,
        ...
        } else if (element.is('.post-ss')) {

            element.addClass('is-revealed');

        } else if (element.is('.post-article')) {
            ...
            tl.staggerFromTo(
                element.find('.post-article--hash, .post-article--title, .post-article--subtitle, .post-article--markup > *'),
                1,
                {
                    y: 50,
                    alpha:0,
        ...
        } else if (element.is('.choices')) {

            element.addClass('is-revealed');


            var $choice = element.find('.choice');
            $choice.each(function(index){
                ...
                setTimeout(function(){
                    ...
                    tl.to(
                        $self.find('.choice--subtitle > span'),
                        0.8,
                        {
                            y: '0',
                            alpha:1,
                    ...
                    $self.find('.btnca').addClass('btnca__active');
                    setTimeout(function(){
                        $self.find('.btnca').addClass('btnca__end');
                    }, 1100);
            ...
            tl.to(
                element.find('.homenav--subtitle > div'),
                0.4,
                {
                    y: '0%',
                    ease: Power3.easeOut
            ...
            tl.to(
                element.find('.homenav--subtitle > span'),
                0.8,
                {
                    y: '0',
                    alpha:1,
            ...
            tl.staggerFromTo(
                element.find('.homenav--nav .menu-item'),
                0.8,
                {
                    y: 20,
                    alpha:0,
...
var thishost = 'bearideas.fr';

/* preprod only *//*
    var preprod = true;
    var hostPreprod = 'bear.eric-huguenin.com';
